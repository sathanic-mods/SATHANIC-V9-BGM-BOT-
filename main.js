   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +919778158839")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 🍬\nFor example: +919778158839 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./SathanicV7")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.yellow(`Connected to sathanic v9 => ` + JSON.stringify(XeonBotInc.user, null, 2)))
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ 𝕾𝖆𝖙𝖍𝖆𝖓𝖎𝖈 ᵇᵘᵍ ᵇᵒᵗ ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: sᴀᴛʜᴀɴɪᴄ-ᴍᴏᴅs `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 919778158839`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: sᴀᴛʜᴀɴɪᴄ\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
